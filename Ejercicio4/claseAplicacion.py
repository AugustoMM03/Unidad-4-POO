...
class Calculadora(object):
    ...
    def __init__(self):
        self.__ventana = Tk()
        self.__ventana.title('Calculadora de Numeros imaginarios')
        ...
        ttk.Button(mainframe, text='i', command=partial(self.ponerNUMERO, 'i')).grid(column=2, row=8, sticky=W)
    ...
    def resolverOperacion(self, operando1, operacion, operando2):
        resultado = 0
        if operacion == '+':
            if 'i' in str(operando1) or 'i' in str(operando2):
                imaginarios = Imaginario(operando1, operando2)
                resultado = imaginarios.imaginarioSuma()
            else:
            ...
            if 'i' in str(operando1) or 'i' in str(operando2):
                imaginarios = Imaginario(operando1, operando2)
                resultado = imaginarios.imaginarioResta()            
            else:
                resultado = float(operando1) - float(operando2)
        elif operacion == '*':
            if 'i' in str(operando1) or 'i' in str(operando2):
                imaginarios = Imaginario(operando1, operando2)
                resultado = imaginarios.imaginarioMultiplicacion()
            else:
                resultado = float(operando1) * float(operando2)
        elif operacion == '/':
            if 'i' in str(operando1) or 'i' in str(operando2):
                imaginarios = Imaginario(operando1, operando2)
                resultado = imaginarios.imaginarioDivision()            
            else:
                resultado = float(operando1) / float(operando2)
